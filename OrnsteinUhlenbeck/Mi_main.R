...
BridgeChaos=function(a,b,thetaOU,sigmaOU,n,delta,nb,M)
{
  ...
  for(i in 1:nb){
    ...
    X_1=sqrt(2)*sigmaOU*(i*pi*exp(-thetaOU*TiempoC)-i*pi*cosenos[i,]+thetaOU*TF*senos[i,])/(sqrt(1/TF)*((i*pi)^2+(thetaOU*TF)^2))
    Y_1=rbind(Y_1,c((TF-TiempoC[1:(TL-1)])*Integrate_Xms(1/(TF-TiempoC[-TL]),X_1),0))
