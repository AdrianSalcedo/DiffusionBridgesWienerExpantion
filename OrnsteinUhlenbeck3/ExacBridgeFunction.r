...
{
  X=path_OU(a,delta,n,thetaOU,sigmaOU)
  bridge=numeric(n+1)
  ban=0
  while(ban==0){
    Y=rev(path_OU(b,delta,n,thetaOU,sigmaOU))
    if(X[1]<=Y[1])
    {
...
path_OU=function(a,delta,n,theta,sigma)
{
  X=numeric(n+1)
  X[1]=a
  Wt = matrix(0,1,n+1)
  for (k in 2:(n+1)){
    Wt[k] = Wt[k-1] + sqrt(delta)*rnorm(1,0,1)
  }
  for(i in 1:n)
  {
    DeltaW = Wt[i+1]-Wt[i]
    X[i+1]=X[i]-theta*X[i]*delta+sigma*DeltaW
  }
  return(X)
