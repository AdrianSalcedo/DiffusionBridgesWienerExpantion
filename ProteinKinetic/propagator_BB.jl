...
path0 = "Dropbox\\Artículos\\Trabajos_Francisco\\New_ProteinK\\"
path2 = "C:\\Users\\Usuario1\\"
path = path2 * path0
pathd = "D:\\Propagador\\SPK\\"
...
################################################################################
################################################################################
problem = ODEProblem(Prop_order12_BM1000,u0,Time)
    Solution = solve(problem,AutoTsit5(Rosenbrock23()),reltol=1e-15,saveat = step)
    df = DataFrame(Solution) 
    Data = Matrix(df)
CSV.write(pathd * "Propagator_order12_int0.8_MB1000.csv",df)
