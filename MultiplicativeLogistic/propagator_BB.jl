...
path0 = "Dropbox\\Artículos\\Trabajos_Francisco\\New_Logistic\\"
path2 = "C:\\Users\\Usuario1\\"
#path2 = "C:\\Users\\DELL\\"
path = path2 * path0
pathd = "D:\\Propagador\\Logistic\\"
...
################################################################################
################################################################################
problem = ODEProblem(Prop_order12_MB1000,u0,Time)
    Solution = solve(problem,AutoTsit5(Rosenbrock23()),reltol=1e-8,saveat = step)
    df = DataFrame(Solution) 
    Data = Matrix(df)
CSV.write(pathd * "Propagator_order12_int0.1_MB1000.csv",df)
